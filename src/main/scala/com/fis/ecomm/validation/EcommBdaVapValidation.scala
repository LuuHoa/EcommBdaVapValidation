...
object EcommBdaVapValidation extends Serializable with App {
  ...
  printf("EcommBdaVapValidation::job is started at %s", start_time)
  ...
  val query_p2="""
    WITH failedrun as (
         select table_name, count_date, runtime_sql
         from
  ...
  import spark.implicits._
  var saved_df = Seq.empty[targetSchemeTarget].toDF()
  logger.info("Starting with part I - today's validation: ")
  tables_par_array_list_p1.foreach {
    eachrow =>
      val table_id = eachrow.getInt(1)
      val table_name = eachrow.getString(2)
      var date_column_name=eachrow.getString(5)
      val ANYnum_days = eachrow.get(7)
      var num_days = 0
      if (ANYnum_days != null) { num_days = ANYnum_days.toString.toInt }
  ...
  saved_df.show(false)
  saved_df.coalesce(1).write.mode("append").parquet(target_path+"/extract_date="+run_date_formatted)
  refreshTable()

  def runCount(table_id: Int, table_name: String, count_date: String, date_column_name: String, runtime_sql: String) = {
    ...
    try {
      ...
      saved_df = saved_df.unionAll(vap_vs_bda_count_df)
    }
    catch {
      case e: Throwable =>
        println(e)
        ...
        import spark.implicits._
        var bda_count_failed_df= Seq(targetSchemeTarget(1,1,"1","1","1","1","1", table_id, table_name, count_date,  0, 0, "N", 0 , date_column_name, inserted_time, runtime_sql+" has failed")).toDF

        saved_df = saved_df.unionAll(bda_count_failed_df)
